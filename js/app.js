...
const App = (() => {
  ...
  const dom = {
    ...
    langTrigger: document.querySelector(".lang-trigger"),
  ...
  async function initSemanticSearch() {
    try {
      if (dom.searchHint) {
        ...
        dom.searchHint.textContent = t.semanticLoading || "正在加载语义模型…";
      }
      await buildEmbeddings();
      semanticReady = true;
      if (dom.searchHint) {
        const t = I18N[currentLang] || I18N.zh;
        dom.searchHint.textContent = t.semanticReady || "语义搜索已就绪 ✓";
      }
    } catch (_) {
      semanticReady = false;
      if (dom.searchHint) {
        ...
        dom.searchHint.textContent =
          t.searchHint || "输入自然语言描述，实时匹配能力锚点";
      }
    }
  }
  ...
  function renderCards() {
    ...
    if (filtered.length === 0) {
      ...
      if (query && currentCategory !== "all") {
        ...
        dom.emptySub.textContent = (t("emptyCatHint") || "")
          .replace("{query}", query)
...
# evidence: { count: ${cap.evidence ? cap.evidence.count : 0}, uncertainty: ${cap.evidence ? cap.evidence.uncertainty.toFixed(2) : 0.1} }`;
  ...
  // S12-B：地域检测（通过 /api/region 获取 IP 归属地国家代码）
  ...
  async function detectRegion() {
    if (regionResolved) return cachedRegion;
  ...
  function mapRegionToLang(country) {
    const map = {
      JP: "ja",
  ...
  // S12-B：语言优先级 URL ?lang= > localStorage > 浏览器语言 > 地域推断 > 默认 zh

  function resolveInitialLang() {
    return getUrlLang() || readStoredLang() || detectBrowserLang() || "zh";
  }

  async function resolveInitialLangAsync() {
    ...
    if (browserLang !== "zh") return browserLang;

    try {
      const country = await detectRegion();

      const regionLang = mapRegionToLang(country);

      if (regionLang && I18N[regionLang]) return regionLang;
    } catch (_) {
      /* 静默降级 */
    }
  ...
  function applyLang(lang) {
    ...
    // S12-C：更新 Globe 下拉菜单当前语言标签

    var labelMap = { zh: "ZH", en: "EN", ja: "JA" };

    if (dom.langCurrentLabel) {
      dom.langCurrentLabel.textContent = labelMap[lang] || lang.toUpperCase();
    }

    // 更新下拉选项 active 状态

    if (dom.langMenu) {
      dom.langMenu.querySelectorAll(".lang-option").forEach(function (opt) {
    ...
    // S12-A：仅首页才有搜索/Federation 区域，子页面跳过

    if (dom.subtitle) dom.subtitle.textContent = t("subtitle");

    if (dom.searchInput) dom.searchInput.placeholder = t("searchPlaceholder");

    if (dom.searchHint) dom.searchHint.textContent = t("searchHint");

    // Federation panel（仅首页）

    if (dom.federatedToggleText) {
      dom.federatedToggleText.textContent = federated
        ? t("federationToggle") + " ✓"
        : t("federationToggle");
    }

    if (dom.federationTitle) dom.federationTitle.textContent = t("federation");

    if (dom.federationAddNodeTitle)
      dom.federationAddNodeTitle.textContent = t("federationAddNode");

    if (dom.federationNodesTitle)
      dom.federationNodesTitle.textContent = t("federationNodes");

    if (dom.fedNodeId) dom.fedNodeId.placeholder = t("federationNodeId");

    if (dom.fedNodeName) dom.fedNodeName.placeholder = t("federationNodeName");

    if (dom.fedNodeEndpoint)
      dom.fedNodeEndpoint.placeholder = t("federationNodeEndpoint");

    if (dom.fedNodeDesc) dom.fedNodeDesc.placeholder = t("federationNodeDesc");

    if (dom.fedNodeTrust)
      dom.fedNodeTrust.placeholder = t("federationNodeTrust");

    if (dom.fedAddBtn) dom.fedAddBtn.textContent = t("federationAddBtn");

    if (dom.federationNoNodes)
      dom.federationNoNodes.textContent = t("federationNoNodes");

    if (dom.federatedToggle) renderFederationNodes();

    updateAuthUI();

    if (dom.categoryFilter) renderCategoryChips();

    if (dom.cardGrid) renderCards();
  ...
  // S12-C：Globe 语言下拉菜单开关

  function toggleLangMenu() {
    if (!dom.langDropdown) return;

    var open = dom.langDropdown.classList.contains("open");
    ...
    if (open) {
      ...
      if (dom.langTrigger)
        dom.langTrigger.setAttribute("aria-expanded", "true");
    }
  }
  ...
  function closeLangMenu() {
    ...
    if (dom.langTrigger) dom.langTrigger.setAttribute("aria-expanded", "false");
  }
  ...
  // S12-FIX：document 级"点击外部关闭"（health/api-docs 页无 modalOverlay，需全局监听）

  function handleDocumentClick(e) {
    if (!dom.langDropdown) return;

    if (!dom.langDropdown.contains(e.target)) closeLangMenu();
  }
  ...
  async function init() {
    // S12-B：异步语言检测（含地域推断，仅首次加载）

    var initialLang = await resolveInitialLangAsync();

    applyLang(initialLang);

    // S12-A：仅首页才初始化搜索/模态/联邦等交互组件
    ...
    // S12-FIX：三页通用的点击外部关闭语言下拉

    document.addEventListener("click", handleDocumentClick);

    // Federation（仅首页）

    if (dom.federatedToggle) {
    ...
    if (dom.searchSuggestions) {
      dom.searchSuggestions.addEventListener("click", (e) => {
        ...
        if (query) {
          ...
          dom.searchInput.focus();
          handleSearch();
          dom.searchInput.scrollIntoView({
            behavior: "smooth",
            block: "center",
          });
        }
